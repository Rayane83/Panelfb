...
export function DashboardSummaryFixed({ guildId, currentRole, entreprise }: DashboardSummaryProps) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null);
      try {
        const API_BASE = 'https://repo-optimizer-3.preview.emergentagent.com';
        
        // Add default enterprise
        ...
        try {
          ...
          const summary = await apiGet(`${API_BASE}/api/dashboard/summary/${guildId}?entreprise=${encodeURIComponent(defaultEnterprise)}`);
          
          if (summary) {
            results.push({
              entreprise: defaultEnterprise,
